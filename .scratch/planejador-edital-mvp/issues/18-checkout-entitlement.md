...
- [x] A interface mostra versão do plano, preço, periodicidade, limites, renovação e condições antes do checkout.
- [x] O checkout usa provedor externo sem armazenar número completo de cartão ou código de segurança.
- [x] Webhook válido e estado confirmado concedem exatamente uma vez os entitlements contratados.
- [x] Webhook inválido, repetido ou fora da tolerância não altera acesso.
- [x] O backend bloqueia recurso restrito sem entitlement mesmo que a UI seja manipulada.
- [x] Testes de contrato do provedor executam sandbox real; integrações locais usam PostgreSQL e Redis reais, sem mocks.
...
- 2026-08-19 — Concluída na `main`: commits `0a55bd5` e merge `0ec491a`. Implementado grace period `past_due` de 3 dias. Verificações: `pnpm test` (59 testes base, 78 API e 27 web aprovados), `pnpm typecheck`, integração real Stripe sandbox + PostgreSQL/Redis (3/3 aprovados) e Docker com PostgreSQL/Redis/MinIO (9 aprovados, 1 opt-in ignorado). Critérios encerrados após integração.
